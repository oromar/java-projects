package com.globalsoft.util;

public enum Messages {
	
	Messages;
	
	public static final String INVALID_CPF = "CPF Inválido";
	public static final String INVALID_NAME = "Nome Inválido";
	public static final String INVALID_LOGIN = "Login Inválido";
	public static final String INVALID_PASSWORD = "Senha Inválida";
	public static final String INVALID_DATE = "Data Inválida";
	public static final String INVALID_ROLE = "Perfil Inválido";
	public static final String CPF_EXISTS_ALREADY = "Já existe um usuário com o CPF cadastrado";
	public static final String LOGIN_IN_USE_ALREADY = "Login já em uso, favor escolha outro login";
	public static final String CANNOT_PASS_NULL_OBJECT = "Não pode ser passado um objeto nulo !";
	public static final String 	ARE_YOU_SURE_TO_EXIT = "Você tem certeza que deseja sair ?";
	

}

